...
app.get("/api/admin/chat-quotas", async (req, res) => {
  try {
    const authHeader = req.headers.authorization || "";
    const token = authHeader.replace("Bearer ", "") || (req.query.token as string) || "";
    if (!token.startsWith("secret_astro_token_sampathub89_") && token !== "sampathub89@gmail.com") {
...
app.post("/api/admin/set-chat-limit", async (req, res) => {
  try {
    const { adminToken, userEmail, customLimit } = req.body;
    if (!adminToken || (!adminToken.startsWith("secret_astro_token_sampathub89_") && adminToken !== "sampathub89@gmail.com")) {
      return res.status(401).json({ error: "Unauthorized admin access." });
    }
...
app.post("/api/astrology/palmistry", async (req, res) => {
  try {
    const { imageBase64, name, gender, handChoice, userEmail, language } = req.body;
    ...
    const langPrompt = (language === "english")
      ...
      : "Provide all explanations in natural, comforting, deep Sinhala (සිංහල භාෂාවෙන්). Use standard Sri Lankan palmistry terminology (ජීවන රේඛාව, ශීර්ෂ රේඛාව, හෘද රේඛාව, භාග්‍ය රේඛාව, සූර්ය රේඛාව, ග්‍රහ මණ්ඩල).";

    const promptText = `
      You are an expert Sri Lankan Palmistry Specialist & Astrologer ("හෙළ හස්තරේඛා ශාස්ත්‍රඥ").
      The user uploaded an image of their palm for detailed palmistry reading (හස්තරේඛා පරීක්ෂාව).
      ...
      Please examine the palm image carefully (lines, clarity, depth, mounts, special symbols) and generate a structured JSON report with the following keys:

      1. lifeLine: Analysis of Life Line (ජීවන රේඛාව) - vitality, health, longevity, major life events.
      2. headLine: Analysis of Head Line (ශීර්ෂ රේඛාව) - intellect, wisdom, decision making, mental focus.
      3. heartLine: Analysis of Heart Line (හෘද රේඛාව) - emotional nature, relationships, cardiac health, affection.
      4. fateLine: Analysis of Fate/Wealth Line (භාග්‍ය රේඛාව / ධන රේඛාව) - career prosperity, unexpected gains, success timeline.
      5. sunLine: Analysis of Sun/Fame Line (සූර්ය රේඛාව / විද්‍යා රේඛාව) - fame, arts, education, social prestige.
      6. mounts: Analysis of Palm Mounts (ග්‍රහ මණ්ඩල) - Jupiter (ගුරු), Venus (ශුක්‍ර), Saturn (ශනි), Moon (චන්ද්‍ර), Sun (සූර්ය), Mercury (බුධ), Mars (අඟහරු) mounts.
      7. specialSigns: Special Markings & Signs (විශේෂ ලක්ෂණ) - Trident (ත්‍රිශූලය), Star (තාරකා), Cross, Triangle, Fish sign (මත්ස්‍ය ලක්ෂණය), Bracelets (මණිබන්ධ).
      8. overallReading: Overall Palmistry Summary & Future Outlook (සමස්ත පලාපල).
      9. remedies: Recommended Auspicious Remedies & Guidance (ශාන්තිකර්ම සහ උපදෙස්).
    ...
    const response = await generateContentWithRetryAndFallback({
      contents: contents,
      config: {
        systemInstruction: "You are an expert Sri Lankan Palmistry Specialist (\"හෙළ හස්තරේඛා ශාස්ත්‍රඥ\"). Output only JSON matching the requested keys.",
        temperature: 0.3,
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
          ...
          required: ["lifeLine", "headLine", "heartLine", "fateLine", "overallReading"]
        }
      }
    });
    ...
    // Store image if under size limit
    let storedImage = null;
    if (imageBase64.length < 8000000) {
      storedImage = imageBase64;
    }
    ...
    const newPalmReport: any = {
      ...
      contactType: "email",
      contactValue: userEmail || "guest@astro.lk",
      birthDetails: {
        name: name || "Anonymous",
        gender: gender || "Male",
...
async function getReportsAsync(): Promise<any[]> {
  ...
  const filtered = allRecords.filter((r: any) => r.id !== "google_drive_tokens");
  return filtered;
}
...
app.post("/api/admin/login", (req, res) => {
  try {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || "sampathub89@gmail.com";
    const adminPassword = process.env.ADMIN_PASSWORD || "Admin123+";

    // Force strict email check
    ...
    res.json({
      ...
      admin: { email: adminEmail, name: "Sampath (Astrology Admin)" }
    });
  } catch (error: any) {
...
app.get("/api/auth/google/consent", (req, res) => {
  const adminEmail = (process.env.ADMIN_EMAIL || "sampathub89@gmail.com").toLowerCase().trim();
  let adminName = "Sampath UB";
  if (process.env.ADMIN_EMAIL && process.env.ADMIN_EMAIL !== "sampathub89@gmail.com") {
    const parts = process.env.ADMIN_EMAIL.split("@")[0].split(/[._+-]/);
    adminName = parts.map(p => p.charAt(0).toUpperCase() + p.slice(1)).join(" ");
  }
